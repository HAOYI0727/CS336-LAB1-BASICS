...
def run_embedding(
    ...
    # 创建一个临时的 EmbeddingModule 实例
    embedding = EmbeddingModule(num_embeddings=vocab_size, embedding_dim=d_model)
...
def run_multihead_self_attention(
    ...
    """
    Given the key, query, and value rojection weights of a naive unbatched
    ...
    # # 创建模型实例
    # CMHA = CausalMultiHeadAttention(d_model=d_model, n_heads=num_heads)
    
    # with torch.no_grad():
    #     CMHA.wq.weight.copy_(q_proj_weight.to(in_features.device, in_features.dtype))
    #     CMHA.wk.weight.copy_(k_proj_weight.to(in_features.device, in_features.dtype))
    #     CMHA.wv.weight.copy_(v_proj_weight.to(in_features.device, in_features.dtype))
    #     CMHA.wo.weight.copy_(o_proj_weight.to(in_features.device, in_features.dtype))
    ...
    from cs336_basics.J_Causal_multi_head_attention import CausalMultiHeadAttention
    # 创建模型实例
    CMHA = CausalMultiHeadAttention(d_model=d_model, n_heads=num_heads,use_rope=False)
...
def run_multihead_self_attention_with_rope(
    ...
    # # 创建模型实例
    # CMHA_RoPE = CausalMultiHeadAttentionWithRoPE(
    #     d_model=d_model,
    #     n_heads=num_heads,
    ...
    # with torch.no_grad():
    #     # 复制权重
    #     CMHA_RoPE.wq.weight.copy_(q_proj_weight.to(in_features.device, in_features.dtype))
    #     CMHA_RoPE.wk.weight.copy_(k_proj_weight.to(in_features.device, in_features.dtype))
    #     CMHA_RoPE.wv.weight.copy_(v_proj_weight.to(in_features.device, in_features.dtype))
    #     CMHA_RoPE.wo.weight.copy_(o_proj_weight.to(in_features.device, in_features.dtype))
    
    # CMHA_RoPE.eval()
    # return CMHA_RoPE(in_features, token_positions)
    
    from cs336_basics.J_Causal_multi_head_attention import CausalMultiHeadAttention
    # 创建模型实例
    CMHA = CausalMultiHeadAttention(d_model=d_model, n_heads=num_heads,use_rope=True,
    ...
    CMHA.eval()
    with torch.no_grad():
        return CMHA(in_features, token_positions)
...
def run_silu(in_features: Float[Tensor, " ..."]) -> Float[Tensor, " ..."]:
    ...
    from cs336_basics.F_SwiGLU import SwiGLU
    # 创建一个临时的 SwiGLU 模块实例
    swiglu = SwiGLU(d_model=1, d_ff=1)
    output = swiglu.silu(in_features)
    return output
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    """
    from cs336_basics.H_Softmax import softmax
...
def run_save_checkpoint(
    ...
    """
    from cs336_basics.S_Checkpoint import save_checkpoint
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    from cs336_basics.A_BPE_Trainer import train_bpe
